...
document.addEventListener('DOMContentLoaded', function () {
  (function () {
    ...
    let calendar = new Calendar(calendarEl, {
      ...
      // selectHelper: true, // Provide a UI for time slot selection
      eventResizableFromStart: true,
      customButtons: {
        sidebarToggle: {
          text: 'Sidebar'
      ...
      direction: direction,
      initialDate: new Date(),
      navLinks: true, // allows clicking on day/week names to navigate views

      // Set day start and end times
      minTime: '09:00:00', // Day starts at 9:00 AM
      maxTime: '21:00:00', // Day ends at 9:00 PM
    
      // Add the select callback for selecting time ranges
      select: function (info) {
        // Show the event creation form (or sidebar) with pre-filled start and end dates
        ...
        // // Use moment.js to format the selected date and time
        const formattedStart = moment(info.start).format('YYYY-MM-DD HH:mm');
        const formattedEnd = moment(info.end || info.start).format('YYYY-MM-DD HH:mm');

        // Set the start and end dates in the input fields in the desired format
        eventStartDate.value = formattedStart;
        eventEndDate.value = formattedEnd;
        
        // Initialize Flatpickr to the selected date and time
        start.setDate(info.start, true); // Set the selected start date in Flatpickr
        end.setDate(info.end || info.start, true); // Set the selected end date (or start date if no end) in Flatpickr
      },

      // Flatpickr initialization (ensure these exist)
      if (eventStartDate) {
        var start = eventStartDate.flatpickr({
          enableTime: true,
          dateFormat: 'Y-m-d H:i K', // Using 12-hour format with AM/PM
          onReady: function (selectedDates, dateStr, instance) {
            if (instance.isMobile) {
              instance.mobileInput.setAttribute('step', null);
            }
      ...
      eventClick: function (info) {
        eventClick(info); // Handle clicking on an existing event
      },
      
      datesSet: function () {
        modifyToggler(); // Toggle UI adjustments
      },
      
      viewDidMount: function () {
        modifyToggler(); // Adjust UI
      }
    });
